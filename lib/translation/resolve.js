...
function hasAnyApiCredentials(settings) {
  const s = settings || {};
  const str = (v) => (typeof v === "string" ? v : "").trim();
  return (
    str(s.deeplAuthKey).length > 0 ||
    str(s.googleApiKey).length > 0 ||
    str(s.azureTranslatorKey).length > 0 ||
    str(s.libreTranslateBaseUrl).length > 0 ||
    str(s.customTranslateBaseUrl).length > 0
  );
}

/**
 * True if the chosen backend mode has usable credentials (on-device excluded until implemented).
 * @param {object} settings
 */
export function hasConfiguredTranslationBackend(settings) {
  const mode = settings.translationBackend || BackendId.AUTO;
  if (mode === BackendId.ONDEVICE) return false;
  if (mode === BackendId.AUTO) {
    return hasAnyApiCredentials(settings);
  }
  return canUse(settings, mode);
}
